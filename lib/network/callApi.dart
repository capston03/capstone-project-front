...
class CallApi{
  ...
  Future<dynamic> RequestHttp(uri, map) async{
    ...
    var response = await http.post(Uri.parse(uri), headers: {"Content-Type":"application/json"},body: map).timeout(const Duration(seconds: 20),onTimeout: (){
  ...
  Future<dynamic> MultipartRequestForDialog(uri,Map<String,dynamic> map) async{
    var client = new http.Client();
    try{
      http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse(dns+uri));
      ///upfile(사진)을 제외하고서는 request.fields에다가 넣어줍니다
      ///map형식으로 input을 주었기에 iterator를 돌면서 값을 받습니다
      for(String key in map.keys){
        if(!map[key].toString().contains('upfile')) {
          request.fields[key] = map[key] ?? '';
        }
      }
      List<http.MultipartFile> newList = []; //사진 추가 준비 =>여러개 줄 예정이니
      String image = 'upfile';
      //사진 추가 시작
      for(int i=0;i<5;i++){
        String new_image = image+i.toString(); //사진 이름 변수명 받아오기
        if(map[new_image]!=''){ //사진이 비어있지 않다면 추가해주어야한다
          File imageFile = File(map[new_image]); //사진을 File형태로 받아
          var stream = http.ByteStream(imageFile.openRead()); //stream으로 분해
          var length = await imageFile.length(); //파일 ㅅ이즈후
          var multipartFile = http.MultipartFile(new_image,stream,length,filename: basename(imageFile.path)); //넣어줍니다
          newList.add(multipartFile); //그다음 리스트에 추가
        }
      }
      request.files.addAll(newList); //여러개의 사진을 request에 추가
      var response = await request.send().timeout(const Duration(seconds: 20),onTimeout: (){
        throw Exception('네트워크 접속 지연');
  ...
  Future<dynamic> MultipartRequestForASF(uri,Map<String,dynamic> map) async{
    ...
    try{
      ...
      var multipartFile = http.MultipartFile('FILE_NAME',stream,length,filename: basename(image.path)); //넣어줍니다
  ...
  Future<dynamic> MultipartRequestForOneToOne(String filename, String url, int index, Map<String,dynamic> map) async {
  ...
  static void circularLoading(BuildContext context, String message){
    ...
    showDialog(context: context, builder: (circularContext){
      return alert;
  ...
  static Future circularWidgetLoading(BuildContext context, String message){
    ...
    return showDialog(context: context, builder: (circularContext){
      return alert;
    },barrierDismissible: false,
